...
namespace VilelaBot2.Util
{
    static class VolumeCalc
    {
        ...
        public static int[] FindVolume(int[] volume, int index, int globalVol)
        {
            ...
            double distance = Math.Sqrt(255*255*2);
            int[] close = new[] { 0, 0 };
            int qClose = 0;
            int yClose = 0;
            int vClose = 0;
            ...
            for (int m = 0; m < 256; ++m)
            {
                for (int q = 15; q >= 0; --q)
                {
                    ...
                    for (int y = 0; y <= 20; ++y)
                    {
                        for (int v = 255; v >= 0; --v)
                        {
                            int[] vol = CalcPanning(VolumeBase(globalVol, v, cQ), y, m, surround);

                            double current = Math.Sqrt(Math.Pow(vol[0] - volume[0], 2)
                                + Math.Pow(vol[1] - volume[1], 2));
        ...
        breakAll:
            return new[] { qClose, yClose, vClose, mClose, (int)distance,
